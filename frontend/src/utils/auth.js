...
export const register = async (fullname, email, password, password2) => {
    try {
        ...
        /*
            Destructuration : Cette syntaxe utilise la destructuration pour extraire directement la propriété data de l'objet retourné par axios.get().
            Pourquoi l'utiliser : axios.get() retourne une promesse qui se résout en un objet contenant plusieurs propriétés, comme data, status, statusText, headers, et config. La propriété data contient la réponse réelle du serveur (le corps de la réponse HTTP). En destructurant data, on accède directement à cette information sans avoir besoin d'utiliser response.data par la suite.
        */
...
export const setAuthUser = (access_token, refresh_token) => {
    ...
    Cookies.set("refresh_token", refresh_token, {
        expires: 90, // Refresh token expires in 7 days
        secure: true,
    });
