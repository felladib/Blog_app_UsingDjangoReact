...
function Index() {
    ...
    const stretchedLinkStyle = {
        display: '-webkit-box',
        WebkitLineClamp: 2, // Limite à deux lignes
        WebkitBoxOrient: 'vertical',
        overflow: 'hidden',
        textOverflow: 'ellipsis',
        whiteSpace: 'normal' ,// Assurez-vous que le texte peut aller à la ligne suivante
        wordBreak: 'break-all',
        hyphens: 'auto'
    ...
    const postItemStyle = {
        width: '300px', // Ajustez cette largeur selon vos besoins
    ...
    return (
        <div>
            ...
            <section className="pt-4 pb-0">
                <div className="container">
                    <div className="row">
                        {postItems?.map((p, index) => ( // {postItems?.slice(0, 4).map((p, index)
                            <div className="col-sm-6 col-lg-3" key={index}>
                                <div className="card mb-4">
                                    <div className="card-fold position-relative">
                                        <img className="card-img" style={{ width: "100%", height: "160px", objectFit: "cover" }} src={p.imagePost} alt={p.title} />
                                    </div>
                                    <div className="card-body px-3 pt-3">
                                        <h4 className="card-title">
                                            <Link to={`${p.slug}`} className="btn-link text-reset stretched-link fw-bold text-decoration-none" style={stretchedLinkStyle}>
                                                {/* {p.title?.slice(0, 32) + "..."} */}
                                                {p.title}
                                        ...
                                        <ul className="mt-3 list-style-none" style={{ listStyle: "none" }}>
                                            <li>
                                                <a href="#" className="text-dark text-decoration-none">
                                                    <i className="fas fa-user"></i> {p.profile?.fullname}
                                                </a>
                                            </li>
                                            <li className="mt-2">
                            ...
                            {pageNumbers.map((number) => (
                                <li key={number} className={`page-item ${currentPage === number ? "active" : ""}`}>
                                    <button className="page-link" onClick={() => setCurrentPage(number)}>
                                        {number}
                                        {console.log("the number :",pageNumbers.length)}
                                    </button>
                                </li>
                            ))}
                        </ul>
                            ...
                            <div className="d-flex flex-wrap justify-content-between">
                                {category?.map((c, index) => (
                                    <div className="mt-2" key={index}>
                                        <Link to={`/category/${c.slug}/`} style={{ textDecoration: 'none'  , color:'black'}}>
                                            <div className="card bg-transparent">
                                                <img className="card-img" src={c.imageCtg} style={{ width: "150px", height: "80px", objectFit: "cover" }} alt="card image" />
